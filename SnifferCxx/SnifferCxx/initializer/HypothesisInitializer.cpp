...
namespace Initializer
{
	...
	{
		{"byCell", [](range_t range, int iteration_per_sec)
			{
				return make_shared<UpdateByCell>(range, iteration_per_sec);
			}},
		{"byBlur", [](range_t range, int iteration_per_sec)
			{
				return make_shared<UpdateByConvolution>(range, iteration_per_sec);
			}},
		{"byParticle", [](range_t range, int iteration_per_sec)
			{
				return nullptr;
			}}
	};
		...
		if (hyp_node)
		{
			...
			for (auto hyp : *hyp_node)
			{
				...
				for (auto leak_node : hyp.second)
				{
					...
					transform(node_location.begin(), node_location.end(), leak.location_.begin(), [](ptree::value_type& v)
					          {
