...
namespace Math
{
	...
	double GaussianBlur::gaussian_pdf(const Coordinate& mean, int step, const Coordinate& val)
	{
		...
		auto dists = accumulate(mean.begin(), mean.end(), vector<distribution_t>(), [step](vector<distribution_t> ret_vec, const coord_item_t& item_val)
		                        {
			                        ret_vec.push_back(distribution_t(item_val, step));
			                        return ret_vec;
		                        });
	...
	shared_ptr<Cells> GaussianBlur::blurCell(const Cell& ori_cell, const Map3D& map, int kernel_range)
	{
		...
		for_each(kernel_ref.begin(), kernel_ref.end(), [sum](double& item)
		         {
			         item /= sum;
		         });
		//cout << kernelToString(kernel);
